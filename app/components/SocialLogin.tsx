...
function GoogleIcon(props: React.PropsWithChildren) {
  return (
    <svg
      ...
      fillRule="evenodd"
      clipRule="evenodd"
      className="w-5 h-5 mr-2 overflow-visible"
      {...props}
    >
      <path
  ...
  return (
    <div className="relative mt-10 mb-6">
      ...
      <div className="relative flex justify-center text-sm font-medium leading-6">
        <span className="bg-white dark:bg-zinc-800 px-6 text-gray-900 dark:text-white">
          {children}
        </span>
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <SocialButton
          ...
          label="Twitter"
          icon={<FontAwesomeIcon className="w-5 h-5 mr-2" icon={faTwitter} />}
        />
        <SocialButton
          provider={SocialsProvider.GITHUB}
          label="Github"
          icon={<FontAwesomeIcon className="w-5 h-5 mr-2" icon={faGithub} />}
        />
      </div>
    </div>
  );
